...
export PASTA_BUILD="$PASTA_CODIGOS_LOCAL"
if [ -n "$SUBDIRETORIO_APP" ]; then
    PASTA_BUILD="$PASTA_CODIGOS_LOCAL/$SUBDIRETORIO_APP"
fi
...
if [ "$COMPILAR_NOVAMENTE" = "sim" ]; then
    ...
    if [ "$BUILD_TOOL" = "maven" ]; then
        MSYS_NO_PATHCONV=1 docker run --rm \
          -v "$PASTA_BUILD":/usr/src/app -v "$DIRETORIOBASE/.cache_m2":/root/.m2 -w /usr/src/app \
          maven:3.9-eclipse-temurin-"$JAVA_VERSION" mvn clean package -DskipTests
    else
        MSYS_NO_PATHCONV=1 docker run --rm \
          -v "$PASTA_BUILD":/home/gradle/project -v "$DIRETORIOBASE/.cache_gradle":/home/gradle/.gradle -w /home/gradle/project \
          gradle:8-jdk"$JAVA_VERSION" gradle bootJar --no-daemon
    fi

    JAR_GERADO=$(find "$PASTA_BUILD" -name "*.jar" ! -name "*-sources.jar" | head -n 1)
